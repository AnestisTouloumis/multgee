...
The \code{method} argument specifies whether the standard errors of the regression parameters estimates will be derived from the "sandwich" (\code{method = "sandwich"}) or the "model-based" (\code{method = "naive"}) covariance matrix.
...
The function \code{vcov} returns the "sandwich" (\code{method = "sandwich"}) or the "model-based" (\code{method = "naive"}) covariance matrix.
The default is to use the "sandwich" covariance matrix.
