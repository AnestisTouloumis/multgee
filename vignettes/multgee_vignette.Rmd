...
The \code{robust} argument specifies whether the standard errors of the regression parameters estimates will be derived from the "sandwich" (\code{robust = TRUE}) or the "model-based" (\code{robust = FALSE}) covariance matrix.
...
The function \code{vcov} returns the "sandwich" (\code{robust = TRUE}) or the "model-based" (\code{robust = FALSE}) covariance matrix.
The default is to use the "sandwich" covariance matrix.
It is used internally to calculate Wald-type confidence intervals and perform the comparison between nested models.
...
To decide on the form of the linear predictor, variable selection model procedures could be incorporated using the function \code{waldts}. 
