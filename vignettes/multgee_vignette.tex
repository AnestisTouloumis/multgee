...
covariance matrix of the parameter estimates (\code{vcov}), v) construct
a probability table (to be passed in the core functions) that satisfies
a desired local odds ratios structure (\code{matrixLOR}), and vi) obtain
...
\code{matrixLOR}, \code{gee_criteria}) are described in Section
...
\code{bstart}, \code{LORgee_control}, \code{ipfp.control}, \code{IM}).
...
For completeness, the function \code{gee_criteria} reports commonly used
...
and implemented in \code{intrinsic.pars}. When choosing between two or
...
\code{gee_criteria} can be used to obtain the values for the above
criteria for one or more models of the class \code{LORgee}.
