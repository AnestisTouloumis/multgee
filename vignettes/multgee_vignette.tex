...
\Plaintitle{R Package multgee: A Generalized Estimating Equations Solver for
...
selection (\code{intrinsic.pars}), to constract Wald type confidence
intervals (\code{confint}) and to perform a Wald type goodness-of-fit
test between two nested GEE models (\code{waldts}). We demonstrate the
application of \pkg{multgee} through a clinical trial with clustered
...
consideration. The \code{robust} argument specifies whether the standard
errors of the regression parameters estimates will be derived from the
``sandwich'' (\code{robust = TRUE}) or the ``model-based''
(\code{robust = FALSE}) covariance matrix. The default is to use the
...
The function \code{vcov} returns the ``sandwich'' (\code{robust = TRUE})
or the ``model-based'' (\code{robust = FALSE}) covariance matrix. The
default is to use the ``sandwich'' covariance matrix. It is used
internally to calculate Wald-type confidence intervals and perform the
...
be incorporated using the function \code{waldts}.
